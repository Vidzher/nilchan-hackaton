...
function ResourceContent() {
  ...
  return <div className="mx-auto flex max-w-3xl flex-col gap-6">
    ...
    <div className="rounded-2xl border border-border bg-card p-6 sm:p-7">
      ...
      <div className="mt-7 flex flex-col gap-2 sm:flex-row">{resource.status === 'NOT_COMPLETED' ? <Button nativeButton={false} render={<Link href={`/quiz?id=${resource.id}`} />}><Play className="size-4" aria-hidden="true" />Начать quiz</Button> : null}{resource.status === 'PROCESSING' ? <Button disabled><ListChecks className="size-4" aria-hidden="true" />Quiz создаётся…</Button> : null}{resource.status === 'FAILED' ? <Button variant="secondary" onClick={retry} disabled={retrying}><RotateCw className="size-4" aria-hidden="true" />{retrying ? 'Повторяем…' : 'Повторить создание quiz'}</Button> : null}<Button variant="outline" nativeButton={false} render={<a href={resource.url} target="_blank" rel="noreferrer noopener" />}><ExternalLink className="size-4" aria-hidden="true" />Открыть оригинал</Button></div>
    </div>
  </div>
}
